...
const collect_data = require("./dataCollect_capital_news.js");
...
async function home () {
    let details = [];
    let report = [];
    let primary_url = ["https://classifieds.kelownacapnews.com/classifieds/apartment-rentals",
        "https://classifieds.kelownacapnews.com/classifieds/house-rentals",
        "https://classifieds.kelownacapnews.com/classifieds/roommate"];
    ...
    await custom_function.myForEach(primary_url, async (element, index, arr) => {
        ...
        try {
            ...
            try {
                await page.waitForXPath('//div[@class="umb-12 ad-item item-full"]');
            } catch {

            }
            let product_links = await custom_function.get_data(page, '//div[@class="umb-12 ad-item item-full"]/parent::a', 'href');
    ...
    ads_report['CapitalNews'] = await details.length;
    await report.push(ads_report);

    await save_file.save_file(details, report, "CapitalNews");
